...
def main():
	...
	for i in range(len(new_data_points)):
		...
		if "ROTORCRAFT_FP" in line:
			data_point_ctr += 1
			#print line.split(' ')[3], line.split(' ')[12] #east
			#print line.split(' ')[4], line.split(' ')[13] #noth
			#print line.split(' ')[5], line.split(' ')[14] #Up
			#print line.split(' ')[11], line.split(' ')[15] #Psi
			d_x = float(line.split(' ')[3])*0.0039063
			d_y = float(line.split(' ')[4])*0.0039063
			d_altitude = float(line.split(' ')[5])*0.0039063
