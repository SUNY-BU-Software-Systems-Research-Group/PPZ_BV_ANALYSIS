...
def run_average_report():
	...
	print "ACC_X_ERR\t\t", averages[0], "\t", stddev(values)
	results_output.write("ACC_X_ERR\t\t" + str(averages[0]) +  "\t\t" + str(stddev(values)) +"\n")
...
def main():
	...
	for i in range(len(new_data_points_1)):
		...
		if "NPS_SENSORS_SCALED" == line.split(' ')[2]:
			data_point_ctr_1 += 1
			d_val_list = line.split(' ')

			# roll pitch yaw phi theta psi			
			d_time = float(d_val_list[0])
			d_acc_x = float(d_val_list[3])		#NPS_SENSORS_SCALED->acc_x
			desired_vals.append(d_acc_x)
	...
	for i in range(len(new_data_points_2)):
		...
		if "NPS_SENSORS_SCALED" == line.split(' ')[2]:
			...
			a_speed = float(d_val_list[3])		#GPS->speed
			actual_vals.append(d_acc_x)
