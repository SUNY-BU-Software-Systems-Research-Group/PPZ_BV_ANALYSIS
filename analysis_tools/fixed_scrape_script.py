...
def run_average_report():
	...
	print "PITCH_ERR\t\t", averages[1],  "\t\t", stddev(values[1])
	results_output.write("PITCH_ERR\t\t" + str(averages[1]) +  "\t\t" + str(stddev(values[1])) +"\n")

	print "YAW_ERR\t\t\t", averages[2],  "\t\t", stddev(values[2])
	results_output.write("YAW_ERR\t\t\t" + str(averages[2]) +  "\t\t" + str(stddev(values[2])) +"\n")
...
def main():
	...
	for i in range(len(new_data_points)):
		...
		if "DESIRED" in line:
			...
			j = i+1
			while  j < len(new_data_points) and  new_data_points[j].split(' ')[2] != "ATTITUDE":
				j+=1
			if (j >= len(new_data_points)):
				return
			a_roll =  float(new_data_points[j].split(' ')[3]) * 57.3	#ATTITUDE->phi		(ROLL)
			a_pitch =   float(new_data_points[j].split(' ')[5]) * 57.3	#ATTITUDE->theta	(PITCH)
			#a_yaw = float(new_data_points[j].split(' ')[5]) * 57.3
		
			j = i+1
			while  j < len(new_data_points) and  new_data_points[j].split(' ')[2] != "NAVIGATION":
				j+=1
			if (j >= len(new_data_points)):
				return
			a_x = float(new_data_points[j].split(' ')[5])			#NAVIGATION->pos_x
			a_y = float(new_data_points[j].split(' ')[6])			#NAVIGATION->pos_y

			j = i+1
			while  j < len(new_data_points) and  new_data_points[j].split(' ')[2] != "GPS":		
				j+=1
			if (j >= len(new_data_points)):
				return
			a_yaw = float(new_data_points[j].split(' ')[6])/10		#GPS->course (Or yaw or psi --- in decidegrees, so divded by 10)
			a_altitude = float(new_data_points[j].split(' ')[7])/100	#GPS->alt
			a_airspeed = float(new_data_points[j].split(' ')[8])/100	#GPS->airspeed
			a_climb = float(new_data_points[j].split(' ')[9])/100		#GPS->climb
